...
class Attendance:
    ...
    def __init__(self,root):
        ...
        Left_frame.place(x=20, y=5, width=450, height=550)
        ...
        label_Update_att.place(x=20, y=480, width=180, height=45)
        update_btn = Button(Left_frame, text="Xóa", command=self.delete_data,
                            font=("times new roman", 14, "bold"),
                            bg="red", fg="white", width=250)
        update_btn.place(x=240, y=480,width=180,height=45 )
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=490, y=5, width=740, height=550)
        ...
        search_entry = ttk.Entry(Right_frame,textvariable=self.var_search, width=11, font=("times new roman", 13, "bold"))
    ...
    def importCsv(self):
        global mydata
        mydata.clear()
        # hộp thoại mở tệp của tkinter cho phép người dùng chọn tệp csv
        fln=filedialog.askopenfilename(initialdir=os.getcwd(),title="Open CSV",filetypes=(("CSV File",".csv"),("ALL File","*.*")),parent=self.root)
        with open(fln) as myfile:
            csvread=csv.reader(myfile,delimiter=",") # module csv để đọc dữ liệu từ tệp csv
            for i in csvread:
                mydata.append(i)
                print(mydata)
